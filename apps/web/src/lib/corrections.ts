...
export interface CorrectionPattern {
  ...
  to: string;
  count: number;
  /** 있으면 프롬프트가 from → to 대신 이 문장을 싣는다.
   *
   *  갈래 교정은 from → to 로 적으면 쓸모가 없다. 제목이 자유 텍스트라
   *  「베타 스토어 배포 → 알파 대시보드」 같은 쌍이 매번 새로 생기고, 열세 번
   *  고쳐도 대부분 count 가 1 에서 안 는다 — 모델에게는 일반화할 규칙이 아니라
   *  남의 사연 열세 개다. category 교정이 먹히는 건 값이 여덟 개뿐이라 같은
   *  줄이 반복되기 때문이다.
   *
   *  그래서 갈래는 **갈라짐의 축**으로 적는다. 「같은 도메인이어도 갈래가
   *  다르다 → db.example.com」은 쌓이고, 처음 보는 프로젝트에도 적용된다.
   *  (측정: 대조축 F1 62.6% vs 제목 쌍 55.6% vs 기준선 54.3%.
   *   2회 표본이라 확정은 못 하지만 규칙이 4~5회까지 쌓이는 것이 관측됐다) */
  text?: string;
}
