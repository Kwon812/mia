 ...
 *
 *  5 였다. 어휘 검색(아래)이 닿지 않는 짝이 있어 늘렸다 — "프롬프트 캐싱
 *  효율성"과 "AI 도구 활용법 탐색"은 의미로만 이어져 있어서 tsquery 로는
 *  한 글자도 안 겹친다. 그런 짝은 그냥 **보여주는 것**말고 방법이 없다.
 *
 *  8 인 근거: 지금 이 사용자의 갈래가 7~9개라 사실상 전량이고, 갈래 한 줄이
 *  두 줄(제목+최근)이라 3줄 늘어도 토큰이 무시할 만하다. 갈래가 수십 개로
 *  늘면 이 몫은 다시 무력해지고 그때부터 어휘 검색이 일한다 — 둘은 대체가
 *  아니라 보완이다. */
const ACTIVE_THREAD_RECENT = 8;
 ...
 * 나눌 값어치가 있는 최소 시간.
 *
 * 이 문턱이 재는 것은 "다른 일인가"가 **아니다** — 그건 모델이 판단하고 대개
 * 맞다. 여기서 묻는 것은 **"따로 셀 만한가"**다. 경험 하나로 세는 순간 레벨
 * 공식(√(경험수 × 스킬수))·갈래 생성·지도의 별·성격의 비율·프롬프트의 "최근
 * 3건" 창이 전부 따라 움직이는데, 경험을 다시 합치는 수단은 없다.
 * 못 넘겨도 **버리지 않는다** — 주된 경험의 detail 에 문장으로 남고 스킬도
 * 합쳐진다. 사라지는 것은 "한 건"이라는 카운트뿐이다.
 *
 * 10분이었다. 전송 필터에서 그대로 가져온 값인데 성격이 다르다 — 저쪽은
 * **세션 하나를 통째로 버리는** 문턱이고 여기는 **이미 있는 세션 안에서
 * 나누는** 문턱이다. 241분 세션 안의 7분짜리 다른 프로젝트를 "너무 짧다"고
 * 볼 이유가 저쪽에는 있고 이쪽에는 없다.
 *
 * 실세션 18건에서 같은 모델 출력 위에 문턱만 바꿔 쓸어봤다(호출 없이 재계산):
 *
 *   10분 → 27건    7분 → 30건    5분 → 32건    3분 → 38건
 *
 * 10 → 7 에서 느는 셋이 전부 긴 세션에 묻혀 있던 **다른 프로젝트**다
 * (241분 안의 SOLDIER : A DAY 7분이 여기서 살아난다). 5 로 더 내리면 62분
 * 세션의 5분짜리 곁다리가 들어오고, 3 으로 내리면 1분짜리까지 경험이 된다 —
 * 그건 경유지지 경험이 아니다.
 *
 * 7분에 근거가 하나 더 있다: 압축기가 1분 미만 구간은 칸을 안 주고 접으므로
 * (FOLD_UNDER_SEC), 살아남은 구간은 이미 "1분은 머문 곳"이다. 그게 몇 개
 * 모여 7분이면 스쳐간 것이 아니다.
 */
const MIN_SPLIT_SEC = 7 * 60;
