...
export type Body = {
  ...
  remembered: boolean;
  forgotten: boolean;
  /** 이 경험이 붙은 기억이 **왜 남았는지**. 안 붙었으면 null.
   *  기억 하나에 이유가 여럿일 수 있어(1월에 처음 써봤고 3월에 6건째가 됐다)
   *  가장 센 것 하나를 읽을 때 고른다 — strongestTrigger 가 그 규칙이고,
   *  지도의 색온도도 같은 값을 쓴다. 한 화면에서 두 값이 갈리면 안 된다. */
 ...
 * 그 구분이 없으면 위성 둘이 같은 것으로 보인다.
 *
 * **고른 경험 패널에서만 쓴다.** 겨누기만 할 때(프로브)는 날짜까지다 —
 * 스쳐 지나가며 읽는 자리는 짧아야 하고, 시·분은 "이게 뭔가"보다 "이걸
 * 들여다보는 중"일 때 필요한 값이다.
 *
      ...
      if (found !== hovered) {
        ...
        if (!found || !p) setProbe(null);
          ...
          setProbe({
            ...
            // 자리에서 같은 것을 읽게 된다.
            rows: [
              // **겨눌 때는 날짜까지.** 시각 구간은 고른 뒤에 나온다 —
              // 프로브는 스쳐 지나가며 읽는 것이라 짧아야 하고, 시·분은
              // "이게 뭔가"보다 "이걸 들여다보는 중"일 때 필요한 값이다.
              { k: "날짜", v: ymd(b.occurredAt) },
